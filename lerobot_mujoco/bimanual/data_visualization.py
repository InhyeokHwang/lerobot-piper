...
from mink_ik.bimanual_mink_ik import pick_two_ee_sites, initialize_model


def _vec1(x) -> np.ndarray:
    return np.asarray(x, dtype=np.float64).reshape(-1)

def set_mocap_targets_from_target_state(model, data, target_state: np.ndarray):
    ...
    data.mocap_pos[mocap_l] = lpos
    data.mocap_quat[mocap_l] = lquat
    data.mocap_pos[mocap_r] = rpos
    data.mocap_quat[mocap_r] = rquat
    ...
    with mujoco.viewer.launch_passive(model=model, data=data, show_left_ui=False, show_right_ui=False) as viewer:
        ...
        while viewer.is_running():
            ...
            if "observation.target" in row and row["observation.target"] is not None:
                try:
                    set_mocap_targets_from_target_state(model, data, row["observation.target"])
